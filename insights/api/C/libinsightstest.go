...
// most of this is copied from libapi.go, keep them up to date.
...
import (
	...
	"github.com/ubuntu/ubuntu-insights/insights/api"
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			var got struct {
				Conf    api.Config
				Metrics string
				Flags   api.CollectFlags
			}

			ret := collectCustomInsights(inConfig, inMetrics, tc.flags, func(conf api.Config, metrics string, flags api.CollectFlags) error {
				got.Conf = conf
				got.Metrics = metrics
				got.Flags = flags
				return tc.err
...
func TestUploadImpl(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			var got struct {
				Conf  api.Config
				Flags api.UploadFlags
			}

			ret := uploadCustomInsights(inConfig, tc.flags, func(conf api.Config, flags api.UploadFlags) error {
				got.Conf = conf
				got.Flags = flags
				return tc.err
			})
...
func TestGetConsentImpl(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"unknown state is correctly converted": {
			state: api.ConsentUnknown,
		},

		"false state is correctly converted": {
			state: api.ConsentFalse,
		},

		"true state is correctly converted": {
			state: api.ConsentTrue,
		},
	}
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
...
func TestSetConsentImpl(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			ret := setCustomConsentState(inConfig, tc.state, func(conf api.Config, state bool) error {
				got.Conf = conf
				got.State = state
				return tc.err
			})
