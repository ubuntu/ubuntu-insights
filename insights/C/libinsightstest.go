...
// TestGetSystemOptOutImpl tests the insights_get_system_opt_out_state C binding.
func TestGetSystemOptOutImpl(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, C.bool(tc.want), ret, "Did not get expected system opt-out state")

			require.NotNil(t, got.Conf.Logger, "Logger should not be nil in the callback")
			got.Conf.Logger = nil
			want := testutils.LoadWithUpdateFromGoldenYAML(t, got)
			require.Equal(t, want, got, "C structures should be correctly translated to Go")
		})
	}
}
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.mockErr == nil {
				...
				require.Equal(t, C.GoString(ret), tc.mockErr.Error())
			}
