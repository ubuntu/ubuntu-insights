...
// fakePCollector implements Collector (for several interfaces).
type fakePCollector[T any] struct {
	fn func() (T, error)
}

func (f fakePCollector[T]) Collect(_ platform.Info) (T, error) {
	return f.fn()
}
...
func TestCollect(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Platform, Hardware and Software error warns": {
			hwErr: fmt.Errorf("fake hardware error"),
			swErr: fmt.Errorf("fake software error"),
