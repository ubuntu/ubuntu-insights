...
// If the report timeout is surpased, the upload will stop.
func (um Uploader) BackoffUpload(force bool) (err error) {
	...
	for {
		...
		slog.Warn("Failed to send report, retrying upload after backoff period", "seconds", wait/(1000*1000*1000), "error", err)
		time.Sleep(wait)
	}
