...
func TestUpload(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Errors when not on Windows with bad file permissions": {
			lFiles: map[string]reportType{"1.json": normal}, consent: cTrue, noPerms: true, wantErr: (runtime.GOOS != "windows") && (os.Getuid() != 0), skipContentCheck: true},
...
func TestGetAllSources(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Source with No Folder Perms": {folders: []string{"source"}, noFolderPerms: true, wantErr: (runtime.GOOS != "windows") && (os.Getuid() != 0)},
